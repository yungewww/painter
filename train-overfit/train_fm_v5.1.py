...
STROKE_DIM  = 8
LABEL_MAX   = 15   # 0-15, 16个类 (含person=15)
N_LANDMARKS = 136
...
DATA_ROOT  = "output"
BASE_CKPT  = "model_fm_v4.pt"   # 从v4初始化stage1权重
CKPT       = "model_fm_v5.pt"

# label id → name (与make-data-1000的LABEL_DEF一致)
LABEL_NAMES = [
    "background",      # 0
    "person",          # 1
    "neck",            # 2
        ...
        for json_path in json_paths:
            ...
            for p in [feat_path, text_path, clip_path, dino_tgt_path, lmk_path]:
                ...
                try:
                    ...
                    if feats.ndim != 2 or feats.shape[1] != 384:
                        skipped += 1; continue
                ...
                for i in range(0, N - window, stride):
                    ...
                    label_id  = int(raw_s.get("label_id", 0))
                    label_id  = max(0, min(label_id, LABEL_MAX))  # clamp to [0, LABEL_MAX]
...
class StrokeARFMv5(nn.Module):
    def __init__(self):
        ...
        self.s1_label_head    = nn.Linear(D_MODEL, LABEL_MAX + 1)  # 16 classes (0-15)
...
def train():
    ...
    if os.path.exists(CKPT):
        ...
        for k, v in v4_state.items():
            ...
            # label_head: v4有15类，v5有16类，跳过避免shape不匹配
            elif k.startswith("label_head."):
                pass  # skip: output dim changed 15→16
        ...
        if is_master:
            ...
            if train_stage2:
                ...
                if fm_loss_now < best_loss_s2:
                    ...
                    torch.save(payload, CKPT.replace(".pt", "_s2best.pt"))
