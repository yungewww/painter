...
from PyQt6.QtWidgets import (QApplication, QWidget, QFileDialog, QHBoxLayout,
                              QVBoxLayout, QPushButton, QSpinBox, QDoubleSpinBox, QLabel, QFrame)
...
CKPT_PATH   = r"D:/Github/painter-models/train2/train-2-mdn/base_model_mdn.pt"
...
class MDNHead(nn.Module):
    ...
    def sample(self, h, temperature=1.0):
        ...
        return mu_sel + sigma_sel * temperature * torch.randn_like(mu_sel)
...
class StrokeARMDN(nn.Module):
    ...
    def sample(self, strokes, canvas_feat, temperature=1.0):
        B, T, _ = strokes.shape
        ...
        last = h[:, -1, :]
        return self.mdn_head.sample(last, temperature=temperature)
...
def extract_canvas_feat(clip_model, canvas_pixels, device):
    ...
    with torch.no_grad():
        feat = clip_model.visual(tensor.half()).float()
    return feat  # (1, 512)
...
class InferenceWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Stroke Inference (MDN)")

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"Loading model from {CKPT_PATH}...")
        ckpt = torch.load(CKPT_PATH, map_location=self.device, weights_only=False)
        self.model = StrokeARMDN().to(self.device)
    ...
    def _build_ui(self):
        ...
        ctrl.addWidget(QLabel("笔  Temperature:"))

        self.spin_temp = QDoubleSpinBox()
        self.spin_temp.setRange(0.1, 3.0)
        self.spin_temp.setSingleStep(0.1)
        self.spin_temp.setValue(1.0)
        ctrl.addWidget(self.spin_temp)
    ...
    def _start_generate(self):
        ...
        self.temperature = self.spin_temp.value()
        self.canvas = CanvasBuffer(self.W, self.H)
        self.canvas_widget.canvas = self.canvas
        self.context = []
        self.generating = False
    ...
    def _replay_tick(self):
        if self._replay_idx >= self.n_gt:
            ...
            self.label_status.setText(f"开始推理... temperature={self.temperature}")
    ...
    def _gen_step(self):
        ...
        canvas_feat = extract_canvas_feat(self.clip_model, self.canvas.pixels, self.device)

        seq = torch.tensor([self.context], dtype=torch.float32).to(self.device)
        with torch.no_grad():
            pred = self.model.sample(seq, canvas_feat, temperature=self.temperature)[0].cpu().numpy()
