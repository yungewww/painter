...
def extract_canvas_feat(dino_processor, dino_model, canvas_pixels, device):
...
class InferenceWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Stroke Inference (FM v3 - Blank)")

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"Loading model from {CKPT_PATH}...")
        ckpt = torch.load(CKPT_PATH, map_location=self.device, weights_only=False)
        self.model = StrokeARFMv3().to(self.device)
        self.model.load_state_dict(ckpt["model_state"])
        self.model.eval()

        print(f"Loading DINO from {DINO_DIR}...")
        self.dino_processor = AutoImageProcessor.from_pretrained(DINO_DIR)
        self.dino_model     = AutoModel.from_pretrained(DINO_DIR).to(self.device)
    ...
    def _gen_step(self):
        ...
        canvas_feat = extract_canvas_feat(
            self.dino_processor, self.dino_model, blank, self.device
        )
