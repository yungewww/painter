...
class StrokeARFMv4(nn.Module):
    ...
    def sample(self, strokes, canvas_dino, canvas_clip_patch, text_feat, steps=FM_STEPS):
        ...
        label_pred   = self.label_head(h).argmax(dim=-1)          # (B,)
        lmk_pred     = torch.sigmoid(self.landmark_head(h))       # (B, 136)
        normal_pred  = torch.sigmoid(self.normal_head(h))         # (B, 3)  值域[0,1]，还原: *2-1
...
class InferenceWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Stroke Inference (FM v4)")

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        print(f"Loading model from {CKPT_PATH}...")
        ckpt = torch.load(CKPT_PATH, map_location=self.device, weights_only=False)
        self.model = StrokeARFMv4().to(self.device)
