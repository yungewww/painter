...
class V41TransformerLayer(nn.Module):
    ...
    def forward(self, x, c_canvas, c_text, kv_patch, attn_mask=None, past_kv=None):
        ...
        if past_kv is not None:
            ...
            k_full = torch.cat([k_past, x2], dim=1)
            v_full = torch.cat([v_past, x2], dim=1)
        else:
            k_full = x2; v_full = x2
        past_len = k_full.shape[1] - T
        causal_mask = torch.ones(T, k_full.shape[1], device=x.device, dtype=torch.bool)
        for i in range(T):
            causal_mask[i, past_len + i + 1:] = False
        mha_mask = ~causal_mask
        out, _ = self.self_attn(x2, k_full, v_full, attn_mask=mha_mask, is_causal=False)
        x = x + out
...
class V41Transformer(nn.Module):
    ...
    def forward(self, x, c_canvas, c_text, kv_patch, attn_mask=None, past_kvs=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            x, kv = layer(x, c_canvas, c_text, kv_patch, past_kv=past)
            new_kvs.append(kv)
        return x, new_kvs
...
MAX_KV_WINDOWS = 4   # 推理时保留最近N个window的KV

def trim_kvs(kvs, max_windows=MAX_KV_WINDOWS, window=WINDOW):
...
class StrokeARFMv41(nn.Module):
    ...
    def sample(self, strokes, canvas_dino, canvas_clip_patch, text_feat,
        ...
        label_pred   = self.label_head(h).argmax(dim=-1)
        lmk_pred     = torch.sigmoid(self.landmark_head(h))
        normal_pred  = torch.sigmoid(self.normal_head(h))
        return stroke, label_pred, lmk_pred, normal_pred, new_kvs
...
class InferenceWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Stroke Inference (FM v4.1)")

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        print(f"Loading model from {CKPT_PATH}...")
        ckpt = torch.load(CKPT_PATH, map_location=self.device, weights_only=False)
        self.model = StrokeARFMv41().to(self.device)
        ...
        self.context = []
        self.past_kvs = None   # KV cache跨window传递
        self.n_gt = 0
    ...
    def _gen_step(self):
        ...
        with torch.no_grad():
            pred_t, label_t, lmk_t, nrm_t, new_kvs = self.model.sample(
                seq, canvas_dino, canvas_clip, text_feat,
                past_kvs=self.past_kvs, steps=self.fm_steps)
            pred     = pred_t[0].cpu().numpy()
            label_id = label_t[0].item()
            lmk      = lmk_t[0].cpu().numpy()
            nrm      = nrm_t[0].cpu().numpy()
        # 每WINDOW笔更新一次KV cache
        if (self.n_gt + self._gen_count) % WINDOW == WINDOW - 1:
            self.past_kvs = trim_kvs(new_kvs)
