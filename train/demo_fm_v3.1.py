...
def encode_stroke(s, W, H):
    p = s["samples"][0]
    label_norm = s.get("label_id", 0) / LABEL_MAX
    return [
        p["x"] / W,
        p["y"] / H,
        p["pressure"],
...
def decode_stroke(pred, W, H, stroke_idx):
    return {
        ...
        "color": [float(np.clip(pred[5], 0, 1)),
                  ...
                  float(np.clip(pred[7], 0, 1))],  # indices unchanged, label is at pred[8]
        "smoothing": 0.7,
...
class InferenceWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Stroke Inference (FM v3.1 - label)")
        ...
        self.dino_model = torch.hub.load(DINO_MODEL, DINO_NAME)
    ...
    def _gen_step(self):
        ...
        canvas_feat = extract_canvas_feat(
            self.dino_model, self.canvas.pixels, self.device
        )
