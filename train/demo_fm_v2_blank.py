...
<<<<<<< HEAD:train-6-consistant-flow/demo_fm_v2_blank.py
CKPT_PATH   = r"D:\Github\painter\train-6-consistant-flow\base_model_fm_v2.pt"
=======
CKPT_PATH   = "base_model_fm_v2.pt"
>>>>>>> a53315b39a7e2acb47a1bc9d89538764ec4fc9f8:train/demo_fm_v2_blank.py
...
class InferenceWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Stroke Inference (FM v2)")

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(f"Loading model from {CKPT_PATH}...")
        ckpt = torch.load(CKPT_PATH, map_location=self.device, weights_only=False)
        self.model = StrokeARFMv2().to(self.device)
